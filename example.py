    ...
    try:
        async with Client(SERVER_ADDRESS) as client:
            ...
            if "error" in str(goto_result).lower():
                ...
                page_aom_snapshot_result = await client.call_tool("capture_area_snapshot", page_aom_snapshot_params)
                # Log a summary, as the full snapshot can be very large
                if isinstance(page_aom_snapshot_result, dict) and "role" in page_aom_snapshot_result:
                    logger.info("Full page AOM snapshot captured successfully (root role: %s).", page_aom_snapshot_result.get("role"))
                else:
                ...
                if "error" not in str(goto_login_result).lower():
                    ...
                    fill_password_params = {"selector": "#password", "text": "SuperSecretPassword!"}
                    ...
                    get_attribute_params = {"selector": "#username", "attribute_name": "type"}
                    get_attribute_result = await client.call_tool("get_element_attribute", get_attribute_params)
                    logger.info("Get attribute 'type' for #username result: %s", get_attribute_result)

                    # Demonstrate get_element_html for the form
                    get_form_html_params = {"selector": "form#login", "char_limit": 200, "remove_scripts": True}
                    get_form_html_result = await client.call_tool("get_element_html", get_form_html_params)
                    logger.info("Get HTML for form#login (truncated, scripts removed) result: %s", get_form_html_result)

                    # Demonstrate get_element_bounding_box for the login button
                    bounding_box_params = {"selector": "button[type='submit']"}
                    bounding_box_result = await client.call_tool("get_element_bounding_box", bounding_box_params)
                    logger.info("Get bounding box for login button result: %s", bounding_box_result)

                    # Demonstrate capture_area_snapshot for the login button
                    button_aom_params = {"selector": "button[type='submit']"}
                    button_aom_result = await client.call_tool("capture_area_snapshot", button_aom_params)
                    if isinstance(button_aom_result, dict) and "role" in button_aom_result:
                        logger.info("Login button AOM snapshot captured successfully (role: %s).", button_aom_result.get("role"))
                    else:
                    ...
                    wait_nav_params = {"url": "**/secure", "timeout": 5, "wait_until": "load"} # wait up to 5s for URL containing /secure
                    nav_after_login_result = await client.call_tool("wait_for_navigation", wait_nav_params)
                    logger.info("Wait for navigation after login result: %s", nav_after_login_result)
                ...
                if "error" not in str(goto_dropdown_result).lower():
                    ...
                    # Select by value
                    select_value_params = {"selector": dropdown_selector, "option_value": "1"}
                    ...
                    # Select by label
                    select_label_params = {"selector": dropdown_selector, "option_label": "Option 2"}
                    select_label_result = await client.call_tool("select_option", select_label_params)
                    logger.info("Select option by label 'Option 2' result: %s", select_label_result)
                ...
                if "error" not in str(goto_hovers_result).lower():
                    ...
                    logger.info("Waiting for caption selector %s to be visible...", caption_selector)
                    # Note: 'state: "visible"' might be needed if element is present but hidden initially
                    await client.call_tool("wait_for_selector", {"selector": caption_selector, "state": "visible", "timeout_ms": 3000}) 
                    logger.info("Waited for %s selector to be visible.", caption_selector)
                    
                    caption_text_params = {"selector": caption_selector}
                    caption_text_result = await client.call_tool("get_text_content", caption_text_params)
                    logger.info("Get text content of revealed caption: %s", caption_text_result.strip() if isinstance(caption_text_result, str) else caption_text_result)
                ...
                logger.info("Navigating back to main page to capture multiple elements: %s", target_url)
                await client.call_tool("goto_page", {"url": target_url})
                
                capture_links_params = {"selector": "ul li a"} # Example: capture all links in lists
                captured_links_result = await client.call_tool("capture_elements", capture_links_params)
                if isinstance(captured_links_result, list) and captured_links_result:
                     logger.info("Captured %d links. First link text: %s", len(captured_links_result), captured_links_result[0].get('innerText', 'N/A'))
                else:
                ...
                logger.info("Final screenshot result (base64 length): %s", len(final_screenshot_result) if isinstance(final_screenshot_result, str) else final_screenshot_result)
            ...
            shutdown_result = await client.call_tool("shutdown", {})
